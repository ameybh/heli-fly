...
const gameScale = 1;
function preload() {
  assets["background"] = loadImage(`assets/sprites/background.png`, (img) => {
  ...
  heliSound = loadSound("./assets/sounds/heli.mp3");
  console.log(assets);
}

function setup() {
  crashSound.playMode("untilDone");
  heliSound.playMode("untilDone");
  heliSound.play();
  ...
  const gameCanvas = createCanvas(gameWrapper.offsetWidth, gameWrapper.offsetHeight);
